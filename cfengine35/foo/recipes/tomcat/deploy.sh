#!/bin/bash
# jenkins.sh
# Tomcat jenkinsment script
#   This script stops tomcat, removes the old WAR, copies the new
#   WAR, then starts tomcat. 
#
#   The 'jenkins' user that runs this script will need appropriate sudo permissions:
...
#
# Usage: jenkins.sh </PATH/TO/WAR>

#EDIT THESE AS REQUIRED
TOMCAT_USER=tomcat                #who to sudo as for the copy
TOMCAT_SERVICE=tomcat             #name of init service script
TOMCAT_INSTANCE=/usr/local/tomcat #tomcat install path (symlink is ok)
PATH=/usr/bin:/bin
...
if ! [[ $# -eq 1 ]]; then 
  echo "USAGE: jenkins.sh </PATH/TO/WAR>"; exit 1
fi
